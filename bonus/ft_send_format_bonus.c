...
/*   Updated: 2021/07/20 00:39:23 by apires-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_check_conversion(const char *format, va_list args, t_option *opt)
{
	if (format[opt->posit] == 'c')
		ft_printf_char(format, args, opt);
	else if (format[opt->posit] == 's')
		ft_printf_str(format, args, opt);
	else if (format[opt->posit] == 'd' || format[opt->posit] == 'i')
		ft_printf_int(format, args, opt);
	else if (format[opt->posit] == 'u')
		ft_printf_unsi_int(format, args, opt);
	else if (format[opt->posit] == 'x')
	{
		opt->hexa = 'x';
		ft_printf_unsi_hex(format, args, opt);
	}
	else if (format[opt->posit] == 'X')
	{
		opt->hexa = 'X';
		...
		ft_printf_point(format, args, opt);
	else if (format[opt->posit] == '%')
		ft_printf_percent(format, opt);
	opt->posit++;
}
